 ...
 * @LastEditTime: 2025-02-26 03:18:55
 * @Description: 
-->
<template>
            ...
            <el-row>
                ...
                <el-col :span="13">
                    <el-form label-width="100px" class="emo-form-inline">
                        <el-form-item label="医生编号">
                            <el-input clearable v-model="requestConfig.form.id" placeholder="请输入订单ID"></el-input>
                        </el-form-item>
                        <el-form-item label="药品ID">
                            <el-input clearable v-model="requestConfig.form.medicineId" placeholder="请输入药品ID"></el-input>
                        </el-form-item>
                    </el-form>
                </el-col>
                <el-col :span="4">
                ...
                <el-col :span="4">
                    <div class="pull-right">
                        <el-button @click="handleAdd" type="danger" :icon="Plus">添加药品订单</el-button>
                    </div>
                </el-col>
            </el-row>
        </div>
    ...
    <el-dialog :title="dialogTitle" v-model="dialogVisible" :close-on-click-modal="false" @close="closeDialog"
        ...
        <el-form :model="dialogConfig" ref="formRef" label-width="120px">
            <el-form-item label="订单ID" prop="id" v-if="isEditModel">
                <el-input v-model="dialogConfig.id" placeholder="请输入订单ID" class="input-field" disabled></el-input>
            </el-form-item>
            ...
            <el-form-item label="药品ID" prop="medicineId">
                <el-input v-model="dialogConfig.medicineId" placeholder="请输入药品ID" class="input-field"></el-input>
            </el-form-item>
            <el-form-item label="购买数量" prop="quantity">
                <el-input v-model="dialogConfig.quantity" placeholder="请输入购买数量" class="input-field"></el-input>
            </el-form-item>
            <el-form-item label="总金额" prop="totalAmount">
                <el-input v-model="dialogConfig.totalAmount" placeholder="请输入总金额" class="input-field"></el-input>
            </el-form-item>
            <el-form-item label="下单时间 :">
                <el-date-picker v-model="dialogConfig.orderTime" type="datetime" placeholder="选择下单时间"
                    format="YYYY-MM-DD HH:mm:ss" value-format="YYYY-MM-DD HH:mm:ss" :clearable="true">
                </el-date-picker>
            </el-form-item>
            <el-form-item label="订单状态" prop="status">
                <el-input v-model="dialogConfig.status" placeholder="请输入订单状态" class="input-field"></el-input>
            </el-form-item>
...
const handleEdit = (id, data) => {
    ...
    dialogConfig.medicineId = data.medicineId;
    dialogConfig.quantity = data.quantity;
    dialogConfig.totalAmount = data.totalAmount;
    dialogConfig.orderTime = data.orderTime;
    dialogConfig.status = data.status;


    dialogVisible.value = true;
    dialogTitle.value = "编辑药品订单";
};
...
const doAddOrEdit = () => {
    if (isEditModel.value) {
        //编辑模式
        let url_param = constant.medicineOrder.updateUrl;
        ...
        //添加模式
        let url_param = constant.medicineOrder.addUrl;
        let data_param = dialogConfig;
...
const table_config = reactive({
    thead: [
        ...
        },
        {
            ...
            type: "function",
            callback: (row, prop) => {
                const data = statusStore.state.orderStatus.filter(
...
const handleDelete = (id) => {
    console.log("删除", id);
    doDeleteRequest(constant.medicineOrder.deleteUrl, id).then((res) => {
        console.log(res);
        if (res.data.code === 200) {
            ElMessage({
                message: "删除成功",
...
const delBatch = () => {
    ...
    if (delIds.length == 0) {
        ...
        doPostRequest(constant.medicineOrder.deleteBatchUrl, delIds).then(
            (res) => {
                console.log(res);
                if (res.data.code === 200) {
                    ElMessage({
...
const searchData = () => {
    ...
    if (isInput) {
        searchOrGetRequest(
            constant.medicineOrder.searchOrGetPageList,
            page_parm,
            data_param
        ).then((res) => {
            if (res.data.code === 200) {
