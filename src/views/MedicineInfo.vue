 ...
 * @LastEditTime: 2025-02-26 03:18:09
 * @Description: 
-->
<template>
            ...
            <el-row>
                ...
                <el-col :span="13">
                    <el-form label-width="100px" class="emo-form-inline">
                        <el-form-item label="药品ID">
                            <el-input clearable v-model="requestConfig.form.id" placeholder="请输入药品ID"></el-input>
                        </el-form-item>
                        <el-form-item label="药品名称">
                            <el-input clearable v-model="requestConfig.form.name" placeholder="请输入药品名称"></el-input>
                        </el-form-item>
                    </el-form>
                </el-col>
                <el-col :span="4">
                ...
                <el-col :span="4">
                    <div class="pull-right">
                        <el-button @click="handleAdd" type="danger" :icon="Plus">添加药品</el-button>
                    </div>
                </el-col>
            </el-row>
        </div>
    ...
    <el-dialog :title="dialogTitle" v-model="dialogVisible" :close-on-click-modal="false" @close="closeDialog"
        ...
        <el-form :model="dialogConfig" ref="formRef" label-width="120px">
            <el-form-item label="药品ID" prop="id" v-if="isEditModel">
                <el-input v-model="dialogConfig.id" placeholder="药品ID" class="input-field" disabled></el-input>
            </el-form-item>
            <el-form-item label="药品名称" prop="name">
                <el-input v-model="dialogConfig.name" placeholder="请输入药品名称" class="input-field"></el-input>
            </el-form-item>
            <el-form-item label="药品类型" prop="type">
                <el-input v-model="dialogConfig.type" placeholder="请输入药品类型" class="input-field"></el-input>
            </el-form-item>
            <el-form-item label="药品单价" prop="price">
                <el-input v-model="dialogConfig.price" placeholder="请输入药品单价" class="input-field"></el-input>
            </el-form-item> 
            <el-form-item label="库存数量" prop="stock">
                <el-input v-model="dialogConfig.stock" placeholder="请输入库存数量" class="input-field"></el-input>
            </el-form-item>
...
//test
// import medicineFormDialog from './medicineFormDialog.vue';
...
const handleEdit = (id, data) => {
    ...
    dialogConfig.type = data.type;
    dialogConfig.price = data.price;
    dialogConfig.stock = data.stock;
...
const doAddOrEdit = () => {
    if (isEditModel.value) {
        //编辑模式
        let url_param = constant.medicine.updateUrl;
        ...
        //添加模式
        let url_param = constant.medicine.addUrl;
        let data_param = dialogConfig;
...
const handleDelete = (id) => {
    console.log("删除", id);
    doDeleteRequest(constant.medicine.deleteUrl, id).then((res) => {
        console.log(res);
        if (res.data.code === 200) {
            ElMessage({
                message: "删除成功",
...
const delBatch = () => {
    ...
    if (delIds.length == 0) {
        ...
        doPostRequest(constant.medicine.deleteBatchUrl, delIds).then(
            (res) => {
                console.log(res);
                if (res.data.code === 200) {
                    ElMessage({
...
const searchData = () => {
    ...
    if (isInput) {
        searchOrGetRequest(
            constant.medicine.searchOrGetPageList,
            page_parm,
            data_param
        ).then((res) => {
            if (res.data.code === 200) {
