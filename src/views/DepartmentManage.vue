 ...
 * @LastEditTime: 2025-02-26 03:15:01
 * @Description: 
-->
<template>
            ...
            <el-row>
                ...
                <el-col :span="13">
                    <el-form label-width="100px" class="emo-form-inline">
                        <el-form-item label="科室ID">
                            <el-input clearable v-model="requestConfig.form.id" placeholder="请输入科室ID"></el-input>
                        </el-form-item>
                        <el-form-item label="科室名称">
                            <el-input clearable v-model="requestConfig.form.name" placeholder="请输入科室名称"></el-input>
                        </el-form-item>
                    </el-form>
                </el-col>
                <el-col :span="4">
                ...
                <el-col :span="4">
                    <div class="pull-right">
                        <el-button @click="handleAdd" type="danger" :icon="Plus">添加科室</el-button>
                    </div>
                </el-col>
            </el-row>
        </div>
    ...
    <el-dialog :title="dialogTitle" v-model="dialogVisible" :close-on-click-modal="false" @close="closeDialog"
        ...
        <el-form :model="dialogConfig" ref="formRef" label-width="120px">
            <el-form-item label="科室ID" prop="id" v-if="isEditModel">
                <el-input v-model="dialogConfig.id" placeholder="请输入科室ID" class="input-field" disabled></el-input>
            </el-form-item>
            <el-form-item label="科室名称" prop="name">
                <el-input v-model="dialogConfig.name" placeholder="请输入科室名称" class="input-field"></el-input>
            </el-form-item>
            <el-form-item label="科室描述" prop="description">
                <el-input v-model="dialogConfig.description" placeholder="请输入科室描述" class="input-field"></el-input>
            </el-form-item>
...
const handleEdit = (id, data) => {
    ...
    dialogConfig.description = data.description;
...
const doAddOrEdit = () => {
    if (isEditModel.value) {
        //编辑模式
        let url_param = constant.department.updateUrl;
        ...
        //添加模式
        let url_param = constant.department.addUrl;
        let data_param = dialogConfig;
...
const handleDelete = (id) => {
    console.log("删除", id);
    doDeleteRequest(constant.department.deleteUrl, id).then((res) => {
        console.log(res);
        if (res.data.code === 200) {
            ElMessage({
                message: "删除成功",
...
const delBatch = () => {
    ...
    if (delIds.length == 0) {
        ...
        doPostRequest(constant.department.deleteBatchUrl, delIds).then(
            (res) => {
                console.log(res);
                if (res.data.code === 200) {
                    ElMessage({
...
const searchData = () => {
    ...
    if (isInput) {
        searchOrGetRequest(
            constant.department.searchOrGetPageList,
            page_parm,
            data_param
        ).then((res) => {
            if (res.data.code === 200) {
