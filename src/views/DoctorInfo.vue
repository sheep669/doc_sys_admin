 ...
 * @LastEditTime: 2025-02-26 02:37:07
 * @Description: 
-->
<template>
            ...
            <el-row>
                ...
                <el-col :span="13">
                    <el-form label-width="100px" class="emo-form-inline">
                        <el-form-item label="医生编号">
                            <el-input clearable v-model="requestConfig.form.id" placeholder="请输入医生编号"></el-input>
                        </el-form-item>
                        <el-form-item label="医生姓名">
                            <el-input clearable v-model="requestConfig.form.name" placeholder="请输入医生姓名"></el-input>
                        </el-form-item>
                    </el-form>
                </el-col>
                <el-col :span="4">
                ...
                <el-col :span="4">
                    <div class="pull-right">
                        <el-button @click="handleAdd" type="danger" :icon="Plus">添加医生</el-button>
                    </div>
                </el-col>
            </el-row>
        </div>
    ...
    <el-dialog :title="dialogTitle" v-model="dialogVisible" :close-on-click-modal="false" @close="closeDialog"
        ...
        <el-form :model="dialogConfig" ref="formRef" label-width="120px">
            <el-form-item label="医生编号" prop="id" v-if="isEditModel">
                <el-input v-model="dialogConfig.id" placeholder="请输入医生编号" class="input-field" disabled></el-input>
            </el-form-item>
            <el-form-item label="医生姓名" prop="name">
                <el-input v-model="dialogConfig.name" placeholder="请输入医生姓名" class="input-field"></el-input>
            </el-form-item>
            <el-form-item label="是否被收藏" prop="isFavorited">
                <el-select v-model="dialogConfig.isFavorited" placeholder="请选择是否收藏" class="input-field">
                    <el-option label="是" value="true"></el-option>
                    <el-option label="否" value="false"></el-option>
                </el-select>
            </el-form-item>
            <el-form-item label="所属科室">
                <el-select v-model="currentDepartmentId" placeholder="请选择" clearable>
                    <el-option v-for="item in statusStore.state.departmentId" :key="item.currentDepartmentId"
                        :label="item.label" :value="item.value" />
                </el-select>
            </el-form-item>
            <el-form-item label="职称" prop="title">
                <el-input v-model="dialogConfig.title" placeholder="请输入职称" class="input-field"></el-input>
            </el-form-item>
            <el-form-item label="医生简介" prop="profile">
                <el-input v-model="dialogConfig.title" placeholder="请输入医生简介" class="input-field"
                    type="textarea"></el-input>
            </el-form-item>
            <el-form-item label="头像URL" prop="avatar">
                <el-input v-model="dialogConfig.avatar" placeholder="请输入头像URL" class="input-field"></el-input>
            </el-form-item>
            <el-form-item label="余额" prop="balance">
                <el-input v-model="dialogConfig.balance" placeholder="请输入余额" class="input-field"></el-input>
            </el-form-item>
...
const currentDepartmentId = ref(1); 
//表单数据
...
const handleEdit = (id, data) => {
    ...
    dialogConfig.name = data.name;
    dialogConfig.gender = data.gender;
    dialogConfig.phone = data.phone;
    dialogConfig.avatar = data.avatar;
    currentDepartmentId.value = parseInt(data.departmentId);
    dialogConfig.address = data.address;
    dialogConfig.balance = data.balance;
...
const doAddOrEdit = () => {
    if (isEditModel.value) {
        //编辑模式
        let url_param = constant.doctor.updateUrl;
        let data_param = dialogConfig;
        console.log(data_param);
        data_param.departmentId = currentDepartmentId.value;
        ...
        //添加模式
        let url_param = constant.doctor.addUrl;
        let data_param = dialogConfig;
...
const table_config = reactive({
    thead: [
        ...
        },
        {
            ...
            type: "function",
            callback: (row, prop) => {
                const data = statusStore.state.departmentId.filter(
        ...
        },
        {
            ...
            type: "function",
            callback: (row, prop) => {
                const data = statusStore.state.isFavorited.filter(
                    (item) => item.value == row[prop]
                );
                if (data && data.length > 0) {
                    return data[0].label;
...
const handleDelete = (id) => {
    console.log("删除", id);
    doDeleteRequest(constant.doctor.deleteUrl, id).then((res) => {
        console.log(res);
        if (res.data.code === 200) {
            ElMessage({
                message: "删除成功",
...
const delBatch = () => {
    ...
    if (delIds.length == 0) {
        ...
        doPostRequest(constant.doctor.deleteBatchUrl, delIds).then(
            (res) => {
                console.log(res);
                if (res.data.code === 200) {
                    ElMessage({
...
const searchData = () => {
    ...
    if (isInput) {
        searchOrGetRequest(
            constant.doctor.searchOrGetPageList,
            page_parm,
            data_param
        ).then((res) => {
            if (res.data.code === 200) {
