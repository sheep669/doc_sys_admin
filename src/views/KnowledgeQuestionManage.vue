 ...
 * @LastEditTime: 2025-02-26 03:22:53
 * @Description: 
-->
<template>
            ...
            <el-row>
                ...
                <el-col :span="13">
                    <el-form label-width="100px" class="emo-form-inline">
                        <el-form-item label="问题ID">
                            <el-input clearable v-model="requestConfig.form.id" placeholder="请输入问题ID"></el-input>
                        </el-form-item>
                        <el-form-item label="问题内容">
                            <el-input clearable v-model="requestConfig.form.question" placeholder="请输入问题内容"></el-input>
                        </el-form-item>
                    </el-form>
                </el-col>
                <el-col :span="4">
                ...
                <el-col :span="4">
                    <div class="pull-right">
                        <el-button @click="handleAdd" type="danger" :icon="Plus">添加问题</el-button>
                    </div>
                </el-col>
            </el-row>
        </div>
    ...
    <el-dialog :title="dialogTitle" v-model="dialogVisible" :close-on-click-modal="false" @close="closeDialog"
        ...
        <el-form :model="dialogConfig" ref="formRef" label-width="120px">
            <el-form-item label="问题ID" prop="id" v-if="isEditModel">
                <el-input v-model="dialogConfig.id" placeholder="请输入问题ID" class="input-field" disabled></el-input>
            </el-form-item>
            <el-form-item label="问题内容" prop="question">
                <el-input v-model="dialogConfig.question" placeholder="请输入问题内容" class="input-field"></el-input>
            </el-form-item>
            <el-form-item label="答案选项" prop="answer">
                <el-input v-model="dialogConfig.answer" placeholder="请输入答案选项" class="input-field"></el-input>
            </el-form-item>
...
//test
// import dailyQuestionsFormDialog from './dailyQuestionsFormDialog.vue';
...
const handleEdit = (id, data) => {
    ...
    dialogConfig.question = data.question;
    dialogConfig.answer = data.answer;
...
const doAddOrEdit = () => {
    if (isEditModel.value) {
        //编辑模式
        let url_param = constant.dailyQuestions.updateUrl;
        ...
        //添加模式
        let url_param = constant.dailyQuestions.addUrl;
        let data_param = dialogConfig;
...
const handleDelete = (id) => {
    console.log("删除", id);
    doDeleteRequest(constant.dailyQuestions.deleteUrl, id).then((res) => {
        console.log(res);
        if (res.data.code === 200) {
            ElMessage({
                message: "删除成功",
...
const delBatch = () => {
    ...
    if (delIds.length == 0) {
        ...
        doPostRequest(constant.dailyQuestions.deleteBatchUrl, delIds).then(
            (res) => {
                console.log(res);
                if (res.data.code === 200) {
                    ElMessage({
...
const searchData = () => {
    ...
    if (isInput) {
        searchOrGetRequest(
            constant.dailyQuestions.searchOrGetPageList,
            page_parm,
            data_param
        ).then((res) => {
            if (res.data.code === 200) {
