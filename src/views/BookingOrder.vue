 ...
 * @LastEditTime: 2025-03-17 17:27:06
 * @Description: 
-->
<template>
            ...
            <el-row>
                ...
                <el-col :span="13">
                    <el-form label-width="100px" class="emo-form-inline">
                        <el-form-item label="缴费ID">
                            <el-input clearable v-model="requestConfig.form.id" placeholder="请输入缴费ID"></el-input>
                        </el-form-item>
                        <el-form-item label="预约ID">
                            <el-input clearable v-model="requestConfig.form.appointmentId"
                                placeholder="请输入预约ID"></el-input>
                        </el-form-item>
                    </el-form>
                </el-col>
                <el-col :span="4">
                ...
                <el-col :span="4">
                    <div class="pull-right">
                        <el-button @click="handleAdd" type="danger" :icon="Plus">添加预约订单</el-button>
                    </div>
                </el-col>
            </el-row>
        </div>
    ...
    <el-dialog :title="dialogTitle" v-model="dialogVisible" :close-on-click-modal="false" @close="closeDialog"
        ...
        <el-form :model="dialogConfig" ref="formRef" label-width="120px">
            <el-form-item label="缴费ID" prop="id" v-if="isEditModel">
                <el-input v-model="dialogConfig.id" placeholder="请输入缴费ID" class="input-field" disabled></el-input>
            </el-form-item>
            <el-form-item label="预约ID" prop="appointmentId">
                <el-input v-model="dialogConfig.appointmentId" placeholder="请输入预约ID" class="input-field"></el-input>
            </el-form-item>
            <el-form-item label="支付金额" prop="amount">
                <el-input v-model="dialogConfig.amount" placeholder="请输入支付金额" class="input-field"></el-input>
            </el-form-item>
            <el-form-item label="支付时间 :">
                <el-date-picker v-model="dialogConfig.paymentTime" type="datetime" placeholder="选择支付时间"
                    format="YYYY-MM-DD HH:mm:ss" value-format="YYYY-MM-DD HH:mm:ss" :clearable="true">
                </el-date-picker>
            </el-form-item>
            <el-form-item label="支付状态">
                <el-select v-model="currentPaidStatus" placeholder="请选择" clearable>
                    <el-option
                        v-for="item in statusStore.state.paidStatus"
                        :key="item.currentPaidStatus"
                        :label="item.label"
                        :value="item.value"
                    />
                </el-select>
...
//test
// import appointmentPaymentFormDialog from './appointmentPaymentFormDialog.vue';

const dialogVisible = ref(false); // 控制对话框的显示
const dialogTitle = ref(""); // 对话框标题
const isEditModel = ref(true); // 是否为编辑模式
const currentPaidStatus = ref(0); // currentPaidStatus
//表单数据
...
const handleEdit = (id, data) => {
    ...
    dialogConfig.appointmentId = data.appointmentId;
    dialogConfig.amount = data.amount;
    dialogConfig.paymentTime = data.paymentTime;
    currentPaidStatus.value = parseInt(data.paidStatus);
    dialogVisible.value = true;
...
const doAddOrEdit = () => {
    if (isEditModel.value) {
        //编辑模式
        let url_param = constant.appointmentPayment.updateUrl;
        let data_param = dialogConfig;
        console.log(data_param);
        data_param.paidStatus = currentPaidStatus.value;
        ...
        //添加模式
        let url_param = constant.appointmentPayment.addUrl;
        let data_param = dialogConfig;
...
const table_config = reactive({
    thead: [
        ...
        },
        {
            ...
            type: "function",
            callback: (row, prop) => {
                const data = statusStore.state.paidStatus.filter(
...
const handleDelete = (id) => {
    console.log("删除", id);
    doDeleteRequest(constant.appointmentPayment.deleteUrl, id).then((res) => {
        console.log(res);
        if (res.data.code === 200) {
            ElMessage({
                message: "删除成功",
...
const delBatch = () => {
    ...
    if (delIds.length == 0) {
        ...
        doPostRequest(constant.appointmentPayment.deleteBatchUrl, delIds).then(
            (res) => {
                console.log(res);
                if (res.data.code === 200) {
                    ElMessage({
...
const searchData = () => {
    ...
    if (isInput) {
        searchOrGetRequest(
            constant.appointmentPayment.searchOrGetPageList,
            page_parm,
            data_param
        ).then((res) => {
            if (res.data.code === 200) {
